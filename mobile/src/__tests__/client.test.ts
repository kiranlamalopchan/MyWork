...
describe("the API client", () => {
  ...
  it("tells a site without the API apart from a missing thing", async () => {
    ...
    await expect(api("home/")).rejects.toMatchObject({ status: 404, message: expect.stringMatching(/doesn't have the KaamKoRecord app API/) });
    ...
    await expect(api("auth/login/", { method: "POST", body: {}, anonymous: true })).rejects.toMatchObject({ message: expect.stringMatching(/doesn't have the KaamKoRecord app API/) });
  });
});
